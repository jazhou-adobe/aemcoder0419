...
function buildApplyButton(link) {
  ...
  cta.href = link?.href || '/study/how-to-apply';
  cta.className = 'degree-search-cta';
  cta.innerHTML = `<span>${link?.textContent || 'Apply now'}</span><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>`;
  return cta;
}
...
export default function decorate(block) {
  ...
  rows.forEach((row) => {
    ...
    if (heading && heading.tagName === 'H2') {
      ...
      section.append(buildApplyButton(link));
